...
const VisitorCardPDF = ({ visitor, qrImage, isSingle = false }) => (
  <View style={isSingle ? cardStyles.singleCard : cardStyles.card}>
    ...
    >
      <View style={cardStyles.titleContainer}>
        <Text
          style={
            isSingle ? cardStyles.singleVisitorName : cardStyles.visitorName
          }
        >
          {visitor.visitorBukrs}
        </Text>
      </View>
    </View>
      ...
      <View style={cardStyles.infoContainer}>
        ...
        <View style={isSingle ? cardStyles.singleInfoRow : cardStyles.infoRow}>
          ...
          <Text
            style={isSingle ? cardStyles.singleInfoValue : cardStyles.infoValue}
          >
            {visitor.visitorEmail || "No Email Provided"}
          </Text>
        </View>
        <View style={isSingle ? cardStyles.singleInfoRow : cardStyles.infoRow}>
          <Text
          ...
          <Text
            style={isSingle ? cardStyles.singleInfoValue : cardStyles.infoValue}
          >
            {visitor.visitorPhone || "No Phone Number Provided"}
          </Text>
        </View>
      </View>
    </View>
...
export const AllVisitorsPDF = ({ visitors, qrImages }) => {
  const pages = [];
  for (let i = 0; i < visitors.length; i += CARDS_PER_PAGE) {
    pages.push(visitors.slice(i, i + CARDS_PER_PAGE));
  }
        ...
        <Page key={pageIndex} size="A4" style={cardStyles.page}>
          {pageVisitors?.map((visitor, index) => (
            <VisitorCardPDF
              key={index}
              visitor={visitor}
              qrImage={qrImages?.[visitor.regid]}
              isSingle={false}
            />
          ))}
          <Text
...
export const SingleVisitorPDF = ({ visitor, qrImage }) => (
  <Document>
    <Page size="A4" style={cardStyles.singleCardPage}>
      <VisitorCardPDF visitor={visitor} qrImage={qrImage} isSingle={true} />
      <Text
        style={cardStyles.pageNumber}
        render={({ pageNumber, totalPages }) =>
          `Page ${pageNumber} of ${totalPages}`
